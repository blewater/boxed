...
type Tasks struct {
	...
	TasksConfig *cfg.TasksBootstrapConfiguration `bson:"tasksConfig" json:"tasksConfig"`
...
// New gets a new initialized workflow struct
func New(config *cfg.TasksBootstrapConfiguration, workflowName string,
	tasksRunnerNewFunc []TaskRunnerNewFunc) (*Tasks, error) {
	if workflowName == "" {
...
func (workflow *Tasks) InitTasksMemState(config *cfg.TasksBootstrapConfiguration,
...
func (workflow *Tasks) SendTaskUpdateToRemote(taskIndex int, msgText string, errIn error) error {
	...
	return grpc.SendServerTaskProgressToRemote(cfg.Get().GetgRPCSrv(), taskName, msgText, errIn)
}
